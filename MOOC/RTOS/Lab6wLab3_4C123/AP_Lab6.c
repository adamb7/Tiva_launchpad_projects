...
//****You implement this function as part of Lab 6*****
  uint8_t NPI_GetStatus[] =   {SOF, 0x00, 0x00, 0x55, 0x06, 0x53};
...
//****You implement this function as part of Lab 6*****
  uint8_t NPI_Register[] = {SOF, 0x00, 0x00, 0x35, 0x84, 0x00};
	NPI_Register[0] = 5;
	for(uint8_t i = 0; i < 6; i++){
...
//****You implement this function as part of Lab 6*****
  uint8_t NPI_AddCharValue[] = {SOF, 0x08, 0x00, 0x35, 0x82, 0x03, 0x0A, 0x00, 0x00, 0x00, 0x02, 0xF1, 0xFF, 0xBA};
  NPI_AddCharValue[5] = permission;
	NPI_AddCharValue[6] = properties;
	NPI_AddCharValue[11] = (uint8_t)(uuid & 0xFF);
	NPI_AddCharValue[12] = (uint8_t)(uuid >> 8);
	NPI_AddCharValue[0] = 13;
...
//****You implement this function as part of Lab 6*****
  ...
  0x0C,0,0,0};    // FCS 
	...
	for(uint8_t i = 11; i < (11 + strlen); i++){
		NPI_AddCharDescriptor[i] = name[i - 11];
	}
	NPI_AddCharDescriptor[1] = strlen + 6;
	NPI_AddCharDescriptor[7] = strlen;
	NPI_AddCharDescriptor[9] = strlen;
	NPI_AddCharDescriptor[0] = 11 + strlen;
	for(uint8_t i = 0; i < (11 + strlen); i++){
		*(msg + i) = NPI_AddCharDescriptor[i];
	}
	for(uint8_t i = (11 + strlen); i < 32; i++){
		*(msg + i) = 0;
	}
	SetFCS(msg);
}
...
//****You implement this function as part of Lab 6*****
  ...
  0x0C,0,0,0};
	...
	for(uint8_t i = 12; i < (12 + strlen); i++){
		NPI_AddCharDescriptor[i] = name[i - 12];
	}
	NPI_AddCharDescriptor[1] = strlen + 7;
	NPI_AddCharDescriptor[8] = strlen;
	NPI_AddCharDescriptor[10] = strlen;
	NPI_AddCharDescriptor[0] = 12 + strlen;
	for(uint8_t i = 0; i < (12 + strlen); i++){
		*(msg + i) = NPI_AddCharDescriptor[i];
	}
	for(uint8_t i = (12 + strlen); i < 32; i++){
		*(msg + i) = 0;
	}
	SetFCS(msg);
}
...
//****You implement this function as part of Lab 6*****
  ...
  0x77};          // FCS
	...
	NPI_GATTSetDeviceName[0] = 8 + strlen;
	NPI_GATTSetDeviceName[1] = strlen + 3;
  for(uint8_t i = 8; i < (8 + strlen); i++){
		NPI_GATTSetDeviceName[i] = name[i - 8];
	}
	for(uint8_t i = 0; i < (8 + strlen); i++){
		*(msg + i) = NPI_GATTSetDeviceName[i];
	}
	for(uint8_t i = (8 + strlen); i < 36; i++){
		*(msg + i) = 0;
	}
	SetFCS(msg);
}
...
// Tx power level
  ...
  0x77};          // FCS
	...
	NPI_SetAdvertisementData[0] = strlen + 17;
	NPI_SetAdvertisementData[1] = strlen + 12;
	NPI_SetAdvertisementData[6] = strlen + 1;
	for(uint8_t i = 8; i < (8 + strlen); i++){
		NPI_SetAdvertisementData[i] = name[i - 8];
	}
	NPI_SetAdvertisementData[8 + strlen] = 0x05;
	NPI_SetAdvertisementData[9 + strlen] = 0x12;
	NPI_SetAdvertisementData[10 + strlen] = 0x50;
	NPI_SetAdvertisementData[11 + strlen] = 0x00;
	NPI_SetAdvertisementData[12 + strlen] = 0x20;
	NPI_SetAdvertisementData[13 + strlen] = 0x03;
	NPI_SetAdvertisementData[14 + strlen] = 0x02;
	NPI_SetAdvertisementData[15 + strlen] = 0x0A;
	NPI_SetAdvertisementData[16 + strlen] = 0x00;
...
//****You implement this function as part of Lab 6*****
  ...
  0xBB};          // FCS
  NPI_StartAdvertisement[0] = 19;
	NPI_StartAdvertisement[8] = (uint8_t)(interval & 0xFF);
	NPI_StartAdvertisement[9] = (uint8_t)(interval >> 8);
