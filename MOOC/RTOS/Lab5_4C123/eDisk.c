...
enum DRESULT eDisk_Init(uint32_t drive){
  ...
  if(0 == drive){             // only drive 0 is supported
     return RES_OK;
  }
  return RES_ERROR;
...
// **write this function**
	...
	uint8_t *addressPt;
	readAddress = EDISK_ADDR_MIN + (512 * sector);
	addressPt = (uint8_t *) readAddress;
	if (EDISK_ADDR_MAX < readAddress){
		return RES_PARERR;
	}
...
// **write this function**
	...
	writeAddress = EDISK_ADDR_MIN + (512 * sector);
	if (EDISK_ADDR_MAX < writeAddress){
		return RES_PARERR;
	}
	for(int16_t i = 0; i < 128; i++){
		Flash_Write(writeAddress, *(uint32_t*) buff);
