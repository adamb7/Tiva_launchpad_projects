...
#define SWITCH (*(((volatile unsigned long*)0x400253FC))) 
...
void Delay(void){unsigned long volatile time;
  //time = 72725;//sim		//160000; //base  0.1sec
	time = 80025;//real
  while(time){
   time--;
  }
...
int main(void){  unsigned long i,last,now,data;
  ...
  while(1){
    Led = GPIO_PORTF_DATA_R;   // read previous
		data = SWITCH; //switch stats
		if((~data & 0x10) || (~data & 0x01)){ //toggle
			Led = Led^0x02; // toggle red LED
		}
		else{
			Led = Led&~0x02; //led off
		}
    GPIO_PORTF_DATA_R = Led;   // output
		if((data^Led))
		{
			if(i<50){
				...
				Data[i] = GPIO_PORTF_DATA_R&0x13; // record PF0,1,4
				last = now;
				i++;
			}
		}
